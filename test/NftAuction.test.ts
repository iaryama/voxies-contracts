...
describe("Nft Auction Test", async () => {
    ...
    describe("Functionality Tests", async () => {
        ...
        it("Non-owner of Nft should not be able to start Dutch Auction", async () => {
            ...
            await expect(approvalResult).to.emit(vox, "Approval");
            await expect(
                auction.connect(accounts2).startDutchAuction(nftId, startBid, endBid, duration)
        ...
        it("Non-owner of Nft should not be able to start English Auction", async () => {
            ...
            await expect(approvalResult).to.emit(vox, "Approval");
            await expect(
                auction.connect(accounts2).startEnglishAuction(nftId, startBid, duration)
            ).to.be.revertedWith("Not NFT owner");
        });
        ...
        it("User should not be able to place bid in Inactive Dutch Auction", async () => {
            ...
            await expect(auction.connect(accounts2).buyNftFromDutchAuction(nftId, newBid,0)).to.be.revertedWith(
                "Not active auction"
            );
        });
        it("User should not be able to buy sold NFT in Dutch Auction", async () => {
            ...
            await expect(auction.connect(accounts3).buyNftFromDutchAuction(nftId, newBid,0)).to.be.revertedWith("Already sold");
        });
        ...
        it("User should not be able to place bid in Inactive English Auction", async () => {
            ...
            await expect(auction.connect(accounts2).placeBidInEnglishAuction(nftId, newBid,1)).to.be.revertedWith(
                "Not active auction"
            );
        });
